...
def drop_columns(file: pd.DataFrame, drop: str) -> pd.DataFrame:
...
def clean_up(file1: pd.DataFrame, file2: pd.DataFrame, configs) -> list:
	file1 = drop_columns(file1, configs['file1_index'])
	file2 = drop_columns(file2, configs['file2_index'])
